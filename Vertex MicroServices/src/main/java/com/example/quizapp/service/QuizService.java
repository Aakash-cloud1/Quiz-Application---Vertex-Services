...
public class QuizService {
    ...
    public void addQuestion(int quizId, Question q) {
        String sql = "INSERT INTO questions (id, quiz_id, text, option_a, option_b, option_c, option_d, correct_index) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = dataSource.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            stmt.setInt(8, q.getCorrectIndex());
            stmt.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void overwriteQuestions(int quizId, List<Question> questions) {
        try (Connection conn = dataSource.getConnection()) {
            ...
            // Save new questions
            String insertSql = "INSERT INTO questions (id, quiz_id, text, option_a, option_b, option_c, option_d, correct_index) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            try (PreparedStatement insertStmt = conn.prepareStatement(insertSql)) {
                for (Question q : questions) {
                    insertStmt.setInt(1, q.getId());
                    insertStmt.setInt(2, quizId);
                    insertStmt.setString(3, q.getText());
                    insertStmt.setString(4, q.getOptionA());
                    insertStmt.setString(5, q.getOptionB());
                    insertStmt.setString(6, q.getOptionC());
                    insertStmt.setString(7, q.getOptionD());
                    insertStmt.setInt(8, q.getCorrectIndex());
                    insertStmt.addBatch();
                }
